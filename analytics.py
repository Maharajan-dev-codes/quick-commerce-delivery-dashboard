...
def get_connection():
    return mysql.connector.connect(
        host="localhost",
        user="root",
        password="Hustle-24/7",
        database="quick_commerce_analysis_v2"
    )

# ------------------ LOAD BASE DATA WITH NAMES ------------------
def load_base_data():
    conn = get_connection()
    ...
    FROM orders o
    JOIN delivery_events d ON o.order_id = d.order_id
...
def add_derived_columns(df):
    ...
    df['picking_minutes'] = (
        df['picking_end_time'] - df['picking_start_time']
    ).dt.total_seconds() / 60
...
def delay_root_cause(df):
    causes = {
        "Peak Hours": df[df['is_peak'] == 'Peak']['delivery_minutes'].mean(),
        "Non-Peak Hours": df[df['is_peak'] == 'Non-Peak']['delivery_minutes'].mean(),
        "Rain": df[df['weather'] == 'Rain']['delivery_minutes'].mean(),
        "Normal Weather": df[df['weather'] == 'Normal']['delivery_minutes'].mean(),
        "High Item Count (≥5)": df[df['item_count'] >= 5]['delivery_minutes'].mean(),
        "Low Item Count (<5)": df[df['item_count'] < 5]['delivery_minutes'].mean()
    }
...
def micro_zone_performance(df):
    ...
    zone_perf['avg_delivery_time'] = round(zone_perf['avg_delivery_time'], 2)
    zone_perf['sla_percent'] = round(zone_perf['sla_percent'] * 100, 2)
...
def rider_efficiency(df):
    ...
    rider_perf['avg_delivery_time'] = round(rider_perf['avg_delivery_time'], 2)
    rider_perf['sla_percent'] = round(rider_perf['sla_percent'] * 100, 2)
    rider_perf['avg_distance_km'] = round(rider_perf['avg_distance_km'], 2)
...
def warehouse_picking(df):
    return {
        "Avg Picking Time (min)": round(df['picking_minutes'].mean(), 2),
        "Max Picking Time (min)": round(df['picking_minutes'].max(), 2),
        "Picking Contribution %": round(
            (df['picking_minutes'].mean() / df['delivery_minutes'].mean()) * 100, 2
        )
    }

# ==================================================
# 6️⃣ ZONE OPTIMIZATION SIMULATION (FINAL)
# ==================================================
def zone_optimization_simulation(
    ...
    optimized_sla = (zone_df['optimized_delivery'] <= 15).mean() * 100

    return {
        "Zone": zone_name,
        "Base SLA %": round(base_sla, 2),
        "Optimized SLA %": round(optimized_sla, 2),
        "SLA Improvement %": round(optimized_sla - base_sla, 2),
